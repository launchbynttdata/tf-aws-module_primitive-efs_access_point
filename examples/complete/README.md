...
|------|-------------|------|---------|:--------:|
| <a name="input_aws_region"></a> [aws\_region](#input\_aws\_region) | AWS region where the example deploys resources. | `string` | n/a | yes |
| <a name="input_logical_product_family"></a> [logical\_product\_family](#input\_logical\_product\_family) | The logical product family name. | `string` | `null` | no |
| <a name="input_logical_product_service"></a> [logical\_product\_service](#input\_logical\_product\_service) | The logical product service name. | `string` | `null` | no |
| <a name="input_region"></a> [region](#input\_region) | AWS region abbreviation (e.g., use, mde, euw for us-east-1, etc.) | `string` | `null` | no |
| <a name="input_class_env"></a> [class\_env](#input\_class\_env) | Environment class (e.g., dev, qa, prod). | `string` | `null` | no |
| <a name="input_instance_env"></a> [instance\_env](#input\_instance\_env) | Instance environment number for uniqueness. | `number` | `null` | no |
| <a name="input_instance_resource"></a> [instance\_resource](#input\_instance\_resource) | Instance resource identifier. | `string` | `null` | no |
| <a name="input_resource_names_map"></a> [resource\_names\_map](#input\_resource\_names\_map) | A map to define resource naming conventions. | <pre>map(object({<br/>    name       = string<br/>    max_length = number<br/>  }))</pre> | <pre>{<br/>  "efs": {<br/>    "max_length": 255,<br/>    "name": "efs"<br/>  }<br/>}</pre> | no |
| <a name="input_encrypted"></a> [encrypted](#input\_encrypted) | Enable encryption at rest for the EFS file system. | `bool` | `true` | no |
| <a name="input_kms_key_id"></a> [kms\_key\_id](#input\_kms\_key\_id) | The ARN for the KMS encryption key to be used to encrypt the filesystem. | `string` | `null` | no |
| <a name="input_performance_mode"></a> [performance\_mode](#input\_performance\_mode) | The file system performance mode. Valid values: generalPurpose, maxIO. | `string` | `"generalPurpose"` | no |
